...
#[test]
fn create_with_promise_return_then_resume() {
    let node = setup_test_contract(near_test_contracts::nightly_rs_contract());

    let yield_payload = vec![6u8; 16];

    // Hardcoded key under which the resume data id will be written.
    let data_id_key = b"yield data id".to_vec();
    ...
    assert_eq!(
        ...
        "{yield_res:?} unexpected result"
    );

    // Get the yield data id from storage
    let res = node
        .user()
        .function_call(
            "alice.near".parse().unwrap(),
